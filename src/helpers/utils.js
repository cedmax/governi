...
export const defineRanges = (data, ranges) => {
  ...
  const buckets = bucketNumbers(values, ranges.length, max)

  return remapBuckets(buckets, ranges)
}

export const fixKey = (str) => str.toUpperCase().split(' ').join('-')
...
export const fixCurrentGovernment = (govs) =>
  govs.map(({ dal, ...gov }) =>
    !dal
      ? gov
      : {
        ...
        current: true,
        giorni: parseInt(
          (new Date() - new Date(dal)) / (1000 * 60 * 60 * 24),
          10
        )
      }
  )
